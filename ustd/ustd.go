...
//
// 供应商接入本身不在这个包里配置：ustd.provider 只是对 pay_sdks 一段的引用，
...
func runCtx() context.Context {
	if p := ustdRunCtx.Load(); p != nil {
		return *p
	}
	return context.Background()
}

// UstdStart 按配置装配 USDT 资金链路。
//
// 配置段缺失、SDK 段没开、密钥未注入，都只记日志并返回：
// 资金通道没配好不该阻断整机启动，但必须留下可查的痕迹。
func UstdStart(ctx context.Context) {
	if ctx == nil {
		ctx = context.Background()
	}
	setRunCtx(ctx)
...
// UstdStop 摘掉供应商客户端。本包无常驻协程，清空指针即可；可重复调用。
func UstdStop(ctx context.Context) {
	active.Store(nil)
}
...
func currentClient() (*client, error) {
	...
	return nil, errors.New("USDT 通道未启动（检查 ustd.provider 是否引用了已开启的 pay_sdks 段与商户账户）")
}

// publish 把归一后的资金回执广播给下游，键为 util.CallUstdMsg+kind。
//
...
func publish(kind string, res *pay.PayResult) {
	...
	if ok := util.DefaultCallFunc.Do(util.CallUstdMsg+kind, res); !ok {
