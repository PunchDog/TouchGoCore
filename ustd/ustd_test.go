...
func init() {
	// 资金门面按 CallUstdMsg+Kind 广播回执，测试在此收口，
	// 下游业务的落库回调就是同一个形态。
	for _, kind := range []string{"Recharge", "Withdraw", "Query"} {
		util.DefaultCallFunc.Register(util.CallUstdMsg+kind, func(res *pay.PayResult) {
			recorder.mu.Lock()
...
// payCfg 组一份「pay_sdks 登记一段 + ustd 段引用它」的完整配置。
//
...
func startWith(t *testing.T, cfg *config.Cfg) {
	...
	UstdStop(nil)
	recorded()
	UstdStart(corectx.WithCfg(context.Background(), cfg))
	t.Cleanup(func() {
		UstdStop(nil)
		recorded()
	})
...
func TestNewClientFailClosed(t *testing.T) {
	...
	cases := []struct {
		...
		{"没有 ustd 段", func(c *config.Cfg) { c.Ustd = nil }},
		{"通道段没引用 SDK", func(c *config.Cfg) { c.Ustd.Provider = nil }},
		{"引用的段不存在", func(c *config.Cfg) { c.Ustd.Provider.SDK = "ghost" }},
	...
	cfg.Ustd.Provider.Account = "reserve"
	c, ok := newClient(cfg)
	if !ok {
...
func TestOperationsRefuseWhenNotStarted(t *testing.T) {
	...
	if _, err := UstdWithdraw(nil, &pay.PayOrder{OrderNo: "O1", Amount: 1, Address: "a"}); err == nil {
		t.Error("未启动时提现应当被拒")
	}
	if _, err := UstdQuery(nil, "O1"); err == nil {
		t.Error("未启动时查询应当被拒")
	}
	if _, err := UstdAccount(nil, nil); err == nil {
...
func TestOrderPreChecks(t *testing.T) {
	...
	recorded()
	if _, err := UstdWithdraw(nil, &pay.PayOrder{OrderNo: "O1", Amount: 1}); err == nil ||
		!strings.Contains(err.Error(), "地址") {
		t.Errorf("提现缺地址未被拦下: %v", err)
	}
	if _, err := UstdRecharge(nil, &pay.PayOrder{OrderNo: "O1", Amount: 0}); err == nil {
		t.Error("零金额未被拦下")
	}
	if _, err := UstdRecharge(nil, &pay.PayOrder{Amount: 1}); err == nil {
		t.Error("空幂等键未被拦下")
	}
	if _, err := UstdQuery(nil, "   "); err == nil {
		t.Error("空订单号未被拦下")
	}
	if got := recorded(); len(got) != 0 {
