...
// 永远藏住——本仓的示例配置就中过这一枪（原 usdt.contract 值 34 位、版本字节对、
// 校验和不通过）。
const (
...
// TestUsdtRejectsBadChainFieldsWithoutSending 地址或备注不合链上规则的单，
// 必须一次请求都不发出、一条回执都不广播：发出去的最坏结果是钱动了但落错了地方。
func TestUsdtRejectsBadChainFieldsWithoutSending(t *testing.T) {
	f, url := newFakeSupplier(t)
	startWith(t, payCfg(url, allEndpoints()))
	recorded()

	if _, err := UsdtWithdraw(nil, &pay.PayOrder{OrderNo: "O9", Amount: 10, Address: tronRangeAcct, Memo: "8890"}); err == nil {
		t.Fatal("TRC20 带备注的提现应当被拒")
	}
	if _, err := UsdtWithdraw(nil, &pay.PayOrder{OrderNo: "O8", Amount: 10, Address: "TAddr1"}); err == nil {
	...
	if _, err := UsdtWithdraw(nil, &pay.PayOrder{OrderNo: "O7", Amount: 10, Address: tronRangeAcct, Memo: "  "}); err != nil {
...
func TestNewClientRejectsBogusContract(t *testing.T) {
	...
	cfg = payCfg("https://pay.invalid", nil)
	cfg.Usdt.Contract = tronBogusChecksum
