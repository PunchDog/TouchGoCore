...
overview: P0/P1/P2、「全部建议」四批，以及继续执行规划（批次 A–E：ctx Stop、TLS/鉴权测试、App DI Step A/B、示例运维、request_id）均已落地。后续仅剩 DI Step C 移除全局与下游 proto 仓库对齐。
todos:
  - id: fix-tests
...
- `App.Context()` 对外暴露
- **继续执行 A–E**：全部 `Service.Stop(ctx)` 对称；Gin/WS TLS 示例配置；gRPC 鉴权单测；App registry Step A/B；示例协议注册 + systemd + README；`request_id` 文档与兼容说明
...
flowchart TB
  ...
  subgraph later [规划外后续]
    StepC[DI Step C 移除全局]
    Downstream[下游 proto 对齐]
  end
  done --> later
```
...
1. 客户端：单调递增 `request_id`；`pending map[uint64]chan` 等待响应
2. 服务端：响应回填相同 `request_id`
3. 可逐步放开 `streamMu` 为「Send 锁 + Recv 分发 goroutine」
4. 旧客户端无 `request_id` 时：兼容模式仍串行（`request_id == 0`）
...
| 顺序  | 阶段               | 状态 | 说明 |
| --- | ---------------- | ---- | ---- |
| 1   | 阶段 1 测试 + ctx 债务 | **已完成** | Stop(ctx) 对称 |
| 2   | 阶段 2 HTTP/WS TLS | **已完成** | 代码 + 示例 JSON |
| 3   | 阶段 3 gRPC 鉴权     | **已完成** | interceptor + 单测 |
| 4   | 阶段 6 示例/运维       | **已完成** | 协议注册 / systemd / README |
| 5   | 阶段 4 App DI      | **Step A/B 已完成** | Step C 移除全局仍为 major |
| 6   | 阶段 5 request_id  | **本库已完成** | 下游仓库需自行对齐 |
...
- **阶段 1**：`go test ./list/... ./ranking/...` 通过；timer/map/ws Stop 响应 ctx；`Cfg.Validate()` 在配置错误时阻止启动 — **已满足**
- **阶段 2**：配置启用 TLS 后 Gin/WS 可 HTTPS/WSS 访问；禁用时行为与现网一致 — **已满足**（示例默认 enable=false）
- **阶段 3**：未授权 gRPC 连接被拒绝；allowlist/token/mtls 三种模式有集成测试或 example 演示 — **单测已满足**
- **阶段 4**：新代码可不依赖 `config.Cfg_` 完成启动；`go test` 不污染全局状态 — **Step A/B 已满足**（全局仍作 fallback）
- **阶段 5**：并发多请求 RPC 响应不错配；`request_id=0` 旧行为仍可用 — **本库已满足**
- **阶段 6**：示例网关可完整握手 + 协议解析；systemd 模板可直接使用 — **已满足**

---

## 风险与决策点（剩余）

1. **request_id 为破坏性 proto 变更**：下游服务需同一版本升级；未升级时 `request_id=0` 仍走串行兼容
2. **DI Step C**：本库已完成 Step A/B 并保留 fallback；移除 `Cfg_` / `GetApp()` 作为 major bump
3. **gRPC 默认鉴权**：缺省仍为 `none`（启动 WARN）；生产应显式 `allowlist`/`token`/`mtls`，且 `skip_for_intranet: false`
