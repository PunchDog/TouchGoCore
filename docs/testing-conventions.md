...
- `app_test.go:112` `TestApp_ShutdownHonorsPerServiceBudget` 断言每个服务拿到 ≥2s 的关闭预算，
  是真实时钟窗口：2026-09-22 实测「4 次全量套件里 2 次失败，同用例单独连跑 15 次全绿」，
  失败集中在全仓 35 个包并行跑满 20 核时。判它是不是真回归，用 `go test -run TestApp_Shutdown -count=10 .` 复跑，
  不要在套件飞行途中下结论（同一次运行里 `slow-1/2/3 超时(2s)` 那三行 ERROR 是设计预期，不是失败原因）。
- 上表四个重型用例的时间窗在负载高时会被压缩。
- `random/random_dist_test.go` 与 `util/random_dist_test.go` 是**统计门**（卡方拟合优度），
  断言对象是随机样本，原理上就有假失败概率，与其他任何用例都不同。显著性取 α=1e-4 而非 0.05，
  两个文件约 28 个断言合计每轮假失败概率约 3e-3（约 350 轮一次）；实测连跑 10 轮全绿。
  单个卡方越界时先重跑一轮确认，再怀疑发生器实现。
